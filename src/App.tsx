...
export default function App() {
  ...
  return (
    <View style={styles.container}>
      ...
      <Button title="Abfolge 1 Licht" onPress={() => startHandler(connectedDevice,'light,27,0,20000,100',[''],setParts,setTotalParts)} />
      {/*Button zum Senden einer sequenz*/}
      <Button title="Abfolge welle (8 Lichter)" onPress={() => startHandler(connectedDevice,'light,21,0000,15000,960?light,20,0100,15100,960?light,16,0200,15200,960?light,12,0300,15300,960?light,07,0400,15400,960?light,08,0500,15500,960?light,25,0600,15600,960?light,24,0700,15700,960',[''],setParts,setTotalParts)} />

      {/*Button zum Senden einer sequenz*/}
      <Button title="Abfolge an (8 Lichter)" onPress={() => startHandler(connectedDevice,'light,21,0000,5000?light,20,0100,5100?light,16,0200,5200?light,12,0300,5300?light,07,0400,5400?light,08,0500,5500?light,25,0600,5600?light,24,0700,5700',[''],setParts,setTotalParts)} />

      <Button title="Abfolge welle (21 Lichter)" onPress={() => startHandler(connectedDevice,'light,20,0,20000,960?light,21,200,20200,960?light,16,400,20400,960?light,12,600,20600,960?light,7,800,20800,960?light,8,1000,21000,960?light,25,1200,21200,960?light,24,1400,21400,960?light,06,1600,21600,960?light,05,1800,21800,960?light,11,2000,22000,960?light,9,2200,22200,960?light,10,2400,22400,960?light,22,2600,22600,960?light,27,2800,22800,960?light,17,3000,23000,960?light,19,3200,23200,960?light,13,3400,23400,960?light,23,3600,23600,960?light,18,3800,23800,960?light,4,4000,24000,960?',[''],setParts,setTotalParts)} />

      <Button title="Abfolge mit sound" onPress={async () => {
        try {
           startHandler(connectedDevice,'light,20,0000,5000,200', [filePath1,filePath2],setParts,setTotalParts);
        } catch (error) {
      ...
      <Text style={styles.message}>Dateien gesendet: {parts}/{totalParts}</Text>
